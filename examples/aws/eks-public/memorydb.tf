...
#
# When enable_memorydb = true (default for this example), this
# provisions a MemoryDB cluster via the upstream
# `aws-anyscale-memorydb` submodule. The cluster endpoint is
# published as `kubernetes_config.redis_endpoint` in the rendered
# cloud-resource so Anyscale Services head-node fault tolerance
# is wired at registration time (requires Anyscale CLI/SDK
# >= 0.26.99).
#
