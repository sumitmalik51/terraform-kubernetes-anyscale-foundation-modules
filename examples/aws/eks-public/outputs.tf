...
#####################################################################
# Rendered cloud-resource.yaml — provided as a reference. The
# `anyscale cloud register -f <file>` path does not currently accept
# this for K8S compute stacks (CLI 0.26.100 asserts an `aws_config`
# block whose schema isn't documented), so the README and the
# `anyscale_registration_command` output use CLI flags instead.
#####################################################################
